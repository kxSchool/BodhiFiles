...
function ChangeDept(dept)
{
    ...
    url="<%=Link.url("/user_mgr/","user_manage.aspx","","","","deptdd","") %>";
...
<myhead:myhead ID="head1" runat="server" />
   ...
   <div>
 			...
 			<form action="<%= Link.url("/user_mgr/","user_manage.aspx","","set","","","")%>" method="post" name="selform" >
 <div id="Div1">
  <table width="100%" border="0" cellpadding="1" cellspacing="1" class="lvt small">
    <tr class="lvtCol">
      ...
      共 <%=total%> 条，当前第 &nbsp;<font color="red"><%=nowpage%></font>/&nbsp;<%=maxpage%>
            ...
            页，每页 
          <%=pagesize%> 条 <a href="<%= Link.url("/user_mgr/","user_manage.aspx","1","","",dept_id,"") %>">首页</a> <a href="<%= Link.url("/user_mgr/","user_manage.aspx",(nowpage-1).ToString(),"","",dept_id,"") %>">上一页</a> &nbsp;<a href="<%= Link.url("/user_mgr/","user_manage.aspx",(nowpage+1).ToString(),"","",dept_id,"") %>">下一页</a> <a href="<%= Link.url("/user_mgr/","user_manage.aspx",maxpage.ToString(),"","",dept_id,"") %>">尾页</a></td>
        
      </tr> 
          <tr bgcolor=white>
         <td align="right" colspan="10" height="22" class="but1" style="width:100%;">&nbsp;           </td> 
          </tr>
           <tr>
           <td align="right" colspan="10" height="22" class="but1" style="width:100%;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
             <tr bgcolor=white>
               <td><input type="button" value="删除选定用户" onClick="{if(confirm('此操作将用户删除！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='delete';document.selform.submit();return true;}return false;}" name="delbuttion" style="width: 93px; height: 22px"></td>
               <td align="center">选择权限组 &nbsp;
                   <select name="movegroupname">
                     <%=selectvalue %>
                   </select>
                   <input onClick="{if(confirm('此操作将用户添加到 这个组！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='movegroup';document.selform.submit();return true;}return false;}" type="button" value="移动到此组" name="action22"></td>
               <td><table border="0" cellspacing="0" cellpadding="0">
                 <tr>
                   <td>选择部门</td>
                   <td><select name="movetodept" style="height:20px;">
                       <option value="0">部门选择</option>
                       <% =selectsb.ToString() %>
                   </select></td>
                   <td><input onClick="{if(confirm('此操作将用户移动到这个部门！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='movedept';document.selform.submit();return true;}return false;}" type="button" value="移动到此部门" name="action2" style="width: 93px"></td>
                   <td>&nbsp;</td>
                 </tr>
               </table></td>
               <td align="right"><input type="checkbox" name="checkbox" value="checkbox" onClick="javascript:SelectAll()">
反选</td>
             </tr>
           </table></td>
           </tr>
