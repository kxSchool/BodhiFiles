...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["param2"] == null || Request.QueryString["param2"] == "")
        {
            ...
            OdbcParameter param = new OdbcParameter("@userno", OdbcType.VarChar, 20);
            param.Value = userno;
            DataTable usertable = db.GetTable(sql, param);
            if (usertable.Rows.Count < 1)
            {
               db.closedb();
               Fun.goback("","没有该用户信息!","返回");
                Response.End();
            }
            else
            {
        ...
        { //保存信息
            ...
            if (Request.QueryString["param2"].ToString() == "saveedit")
            {
                ...
                OdbcParameter[] updateparam ={ 
                    ...
                    new OdbcParameter("@email",OdbcType.VarChar),
                      ...
                      new OdbcParameter("@postalcode",OdbcType.VarChar),
						...
						new OdbcParameter("@dept_id",OdbcType.Int)
				...
				updateparam[10].Value = Request.Form.Get("deptname").ToString();


                //更新 tbuser
                string upsql = "update tbuser set username=?,sex=?,email=?,phone=?,faxnum=?,mobilephone=?,msn=?,qq=?,postalcode=?,address=?,dept_id=? where userno='"+  Request.Form.Get("edituserno") +"'";
                //Response.Write(upsql);
				//Response.End();
				db.ExecuteNonQuery(upsql, updateparam);


                //判断 如果用户权限组改变了则要更新用户的权限  popedom 表      

                string newpopedomgroup = Request.Form.Get("popedomgroup").ToString();
                if (newpopedomgroup != Request.Form.Get("oldpopedomgroup").ToString())
                { //权限组改变了 并且要 新权限组 


                    //  1. 先清空权限
                    string clear_sql = "update popedom set popedom=0 where popedom.userno=?";
                    OdbcParameter clear_param = new OdbcParameter("@userno", OdbcType.VarChar, 50);
                    clear_param.Value = Request.Form.Get("edituserno").ToString();

                    db.ExecuteNonQuery(clear_sql, clear_param);

                    // 2.   将新组的权限设置应用到该用户上     ////////////////////////////

                    string updatesql = "update popedom set popedom=1  where popedom.userno=?  and popedom.variable in(select distinct variable from popedomgroup g where g.groupname=? and g.popedom=1)";
                    OdbcParameter[] param_movegroup = { 
                        new OdbcParameter("@userno",OdbcType.VarChar,50),
                        new OdbcParameter("@popedomgroup", OdbcType.VarChar, 50)
                     };
                    param_movegroup[0].Value = Request.Form.Get("edituserno").ToString();
                    param_movegroup[1].Value = Request.Form.Get("popedomgroup").ToString();
                ...
                db.closedb();
                 Fun.alert("","信息修改成功",Link.url("/user_mgr/", "user_manage.aspx", "", "", "", "", ""),"返回");
                Response.End();
 ...
 <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
  <tr>
    ...
    <td width="100" nowrap class=small><table>
        <tr>
          <td> <b><font color="#ffffff">编辑用户信息</font></b></td>
        </tr>
    </table></td>
...
<div>
      <form name="form1" method="post" action="<%= Link.url("/user_mgr/","user_infoedit.aspx","","saveedit","","","")%>" onsubmit ="return form_check();">
          ...
          <tr>
            ...
            <tr>
		     ...
		     <tr>
                <td style="height: 24px" align="right" >
                    所属部门：                </td>
                <td style="width: 153px; height: 24px;"> <select name="deptname" style="width: 119px"><%= deptselect %></select>                </td>
          </tr  >
		    <tr>
                <td style="height: 24px" align="right" >
                    所属权限组：                </td>
                <td style="width: 153px; height: 24px;"> <select name="popedomgroup"  style="width: 119px"> 
                <option value="">自定义权限组</option>
                 <%= popedomgroupselect %></select>                </td>
          </tr  >
		  	    <tr>
                <td style="height: 24px" align="right" >
                    注册时间：                </td>
            ...
            <td style="width: 153px; height: 24px;">
              ...
              <tr>
                ...
                <input type="hidden" name="oldpopedomgroup" value="<%= oldpopedomgroup %>" />
