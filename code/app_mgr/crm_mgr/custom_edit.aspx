...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["param2"] == null || Request.QueryString["param2"] =="")
        {
            if (Request.QueryString["param1"] != null)
            {
                ...
                DataTable nt = db.GetTable("select * from tbcustom where id=?", p);
                companyname = nt.Rows[0]["companyname"].ToString();
                username = nt.Rows[0]["username"].ToString();
                postcode = nt.Rows[0]["postcode"].ToString();
                phonecode = nt.Rows[0]["phonecode"].ToString();
                phone = nt.Rows[0]["phone"].ToString();
                province = nt.Rows[0]["province"].ToString();
                capital = nt.Rows[0]["capital"].ToString();
                city = nt.Rows[0]["city"].ToString();
                mainproducts = nt.Rows[0]["mainproducts"].ToString();
                fax = nt.Rows[0]["fax"].ToString();
                mobilephone = nt.Rows[0]["mobilephone"].ToString();
                website = nt.Rows[0]["website"].ToString();
                email = nt.Rows[0]["email"].ToString();
                createdate = nt.Rows[0]["createdate"].ToString();
                userno = nt.Rows[0]["userno"].ToString();
    ...
    protected void SaveEdit(DBClass db)
    {
        ...
        string sql = "update tbcustom set companyname=?,username=?,postcode=?,phonecode=?,phone=?,province=?,capital=?,city=?,mainproducts=?,fax=?,mobilephone=?,website=?,email=?,createdate=?,userno=? where id=?";
        OdbcParameter[] param ={ 
        ...
        param[15].Value = Convert.ToInt32(Request.Form.Get("editid").ToString());
...
<myhead:myhead ID="Myhead1" runat="server" />
 ...
 <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
  <tr>
    ...
    <td width="100" nowrap class=small><table>
        <tr>
          <td> <b><font color="#ffffff">客户编辑</font></b></td>
        </tr>
    </table></td>
 ...
 <div>
    <form name="form1" method="post" action="<%= Link.url("/app_mgr/crm_mgr/","custom_edit.aspx","","saveedit","","","") %>" onsubmit ="return form_check();">
        <table align="center" border="0">
            <tr>
                <td colspan="2" style="height: 27px" >
                  <input type="hidden" name="editid" value="<%= Convert.ToInt32(Request.QueryString["param1"].ToString()) %>" /></td>
          </tr>
          <tr>
...
//调用定位区域函数
	...
	<%if (province!="") {%>selectRegion_m('<%=province%>','ProvinceID',document.all);<%}%>
	<%if (capital!="") {%>selectRegion_m('<%=capital%>','CityID',document.all);<%}%>
	<%if (city!="") {%>selectRegion_m('<%=city%>','County',document.all);<%}%>
</script></td>
              </tr>
              <tr>
			  ...
			  <tr>
                ...
                <td style="width: 153px; height: 30px;">
                  <input type="text" name="userno" style="width: 257px; height: 20px;" value="<%=userno %>" /></td>
              </tr>
			  <tr>
                <td style="height: 30px"> 录入时间：</td>
                <td style="width: 153px; height: 30px;">
                  <input type="text" name="createdate" style="width: 257px; height: 20px;" value="<%=createdate %>" /></td>
              </tr>
              <tr>
                <td>                </td>
