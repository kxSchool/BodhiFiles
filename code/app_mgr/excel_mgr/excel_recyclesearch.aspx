...
<script runat="server" language="C#">
        ...
        else
        {
            ...
            if (Request.QueryString["param2"] == "set")
            {
                ...
                if (Request.Form.Get("sel") == null || Request.Form.Get("sel").ToString() == "")
                {
					...
					db.closedb();
					  Fun.alert("","请先选择表格！",Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx","","","","","") ,"返回");
                    Response.End();
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        sqls = "select tbexcel.*,tbuser.username from tbexcel left join tbuser on tbexcel.userno=tbuser.userno where (tbexcel.filetitle like '%"+ condition+"%' or tbuser.username like '%"+ condition+"%') and tbexcel.isdelete='1' order by uptime desc";
    ...
    protected void DeleteData(DBClass db)
    {
        ...
        //删除表
        string delete_sql = "delete from tbexcel where tbexcel.id in (" + Request.Form.Get("sel").ToString() + ")";
        ...
        db.closedb();
		  Fun.alert("","成功删除至回收站！",Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx","","","","","") ,"返回");
        Response.End();
...
<myhead:myhead ID="Myhead1" runat="server" />
  ...
  <tr>
    ...
    <td nowrap class=small style="width: 100px"><table>
        <tr>
          <td> <b><font color="#ffffff">回收站搜索结果</font></b></td>
        </tr>
    </table></td>
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
                         ...
                         <td width="100%" align="center" valign="top">
                             <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                     ...
                                     <td align="left" valign="middle" bgcolor="#FFFFFF"  style="width: 845px; height: 26px"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                                       <tr>
                                         <td align="left"> <a href="<%= Link.url("/app_mgr/excel_mgr/", "excel_recycle.aspx", "", "", "", "", "") %>"><img src="/images/goback.jpg" style="border:0;" alt="" /></a>
										 <a onClick="{if(confirm('此操作将统计表格放入回收站！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='delete';document.selform.submit();return true;}return false;}" href="#"></a> </td>
                                       </tr>
                                     </table>
                                   </td>
                                 </tr>
 			...
 			<form action="<%= Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx","","set","","","") %>" method="post" name="selform" >
...
<%=contentstring %>
         ...
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="10" height="35" class="but1" style="width: 100%;">
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx","1","search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx",(nowpage-1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx",(nowpage+1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/excel_mgr/","excel_recyclesearch.aspx",maxpage.ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">
                     尾页</a></td>
         </tr>
     </table>   
