...
<script runat="server">
   ...
   public string contentstring;
        ...
        else
        {
            if (Request.QueryString["param2"] == "set")
            {
                if (Request.Form.Get("actiontype").ToString() == "search")
                {
                    ...
                    Response.Redirect(Link.url("/app_mgr/ziliao_mgr/","data_shareallsearch.aspx","","search",condition,"",""));
    ...
    {
        //分页页面显示       
        string sql = "select  tbdata.*,tbuser.username as username,department.deptname from (( tbdata left join tbuser on tbdata.userno=tbuser.userno) left join department on tbuser.dept_id=department.id ) where   tbdata.popedom=10000 order by tbdata.uptime desc";
...
function allsearch()
{
	 url="<%=Link.url("/app_mgr/ziliao_mgr/","data_shareallsearch.aspx","","search","param3_idd","","") %>";
...
<td >
 			...
 			<form action="<%= Link.url("/app_mgr/ziliao_mgr/","data_shareall.aspx","","set","","","")%>" method="post" name="selform2" >
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0"
                                         ...
                                         <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
                                           <tr>
                                             ...
                                             <td class=small nowrap><table border="0" cellpadding="0" cellspacing="0">
                                                 <tr>
                                                   <td align="left" valign="middle" class="calendarNav" style="height: 16px"> 单位共享区 </td>
                                                 </tr>
                                             </table></td>
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
      ...
      <td align="center" colspan="11" height="22" class="but1" style="width:100%;">
      ...
      共  <%=total%> 个资料，当前第 &nbsp;<font color="red"><%=nowpage%></font>/   <%=maxpage%>
            ...
            页，每页 
          <%=pagesize%>个&nbsp; <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_shareall.aspx","1","","","","") %>">首页</a> <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_shareall.aspx",(nowpage-1).ToString(),"","","","") %>">上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_shareall.aspx",(nowpage+1).ToString(),"","","","") %>">下一页</a>&nbsp;
          <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_shareall.aspx",maxpage.ToString(),"","","","") %>">尾页</a></td>
