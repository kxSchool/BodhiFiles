  ...
  public string contentstring;
    ...
    void ShowPage(DBClass db)
    {
        ...
        sqls = " select tbdata.*,tbuser.userno,tbuser.username from tbdata left join tbuser on tbdata.userno=tbuser.userno  where ( tbdata.filetitle like '%" + condition + "%' or tbdata.description like '%" +
           condition + "%'  or username like '%" +  condition + "%') and tbdata.popedom=" + Session["dept_id"].ToString() + "   order by tbdata.uptime desc";
    ...
    <td class=small nowrap><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td align="left" valign="middle" class="small" style="height: 16px;">
                              部门共享搜索结果 <span class="txtGreen">"<%= condition %>"</span>
          </td>
        </tr>
    </table> </td>
  </tr>
...
<td >
 			...
 			<form action="<%= Link.url("/app_mgr/ziliao_mgr/","data_sharesearch.aspx","","set","","","")%>" method="post" name="selform" >
    ...
    <tr  bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
      ...
      共  <%=total%> 个资料，当前第 &nbsp;<font color="red"><%=nowpage%></font>/   <%=maxpage%>
            ...
            页，每页 
          <%=pagesize%>个&nbsp; <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_sharesearch.aspx","1","search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">首页</a> <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_sharesearch.aspx",(nowpage-1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_sharesearch.aspx",(nowpage+1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">下一页</a>&nbsp;
          <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_sharesearch.aspx",maxpage.ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),"","") %>">尾页</a></td>
