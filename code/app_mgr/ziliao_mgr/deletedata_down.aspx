...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		sqls = "select * from tbdataRecycle where id=" + sid;
        popedomgrouptable = db.GetTable(sqls, null);
		fileurl=popedomgrouptable.Rows[0]["fileurl"].ToString();
        DataTable dt = db.GetTable("select tbdataRecycle.*,tbuser.userno,tbuser.username,tbuser.dept_id,department.id,department.deptname from (( tbdataRecycle left join tbuser on tbdataRecycle.userno=tbuser.userno ) left join department on tbuser.dept_id=department.id  ) where fileurl='" + fileurl.ToString() + "'", null);
        if (dt.Rows.Count > 0)
        {
        ...
        {
            Page.Response.Write("下载文件出错,可能文件已被删除！");
            db.closedb();
            Page.Response.End();
        }
        ...
        full_fileurl = ConfigurationSettings.AppSettings["delete_center"].ToString() + fileurl.ToString();
        Page.Response.Clear();
        bool success = ResponseFile(Page.Request, Page.Response, responsename, full_fileurl, 1024000);
        if (!success)
        {
            Response.Write("下载文件出错,可能文件已被删除！");
        }
