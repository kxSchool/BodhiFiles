...
<script runat="server">
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        //页面载入显示
        sqls = "select  tbDataOperateLog.*  from tbDataOperateLog  where opt_username like '%" + condition + "%'  order by opt_time desc";
  ...
  <tr>
    ...
    <td class=small nowrap><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td align="left" valign="middle" class="small" style="height: 16px;">
                              下载日志搜索结果 <span class="txtGreen">"<%= condition %>"</span>
          </td>
        </tr>
    </table> </td>
  </tr>
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
     ...
     <%=contentstring %>
       ...
       <tr>
       <td   bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'" align="center" colspan="10" height="22" class="but1" style="width:100%;"> 共 <%=total%> 条，当前第 &nbsp;<font color="red"><%=nowpage%></font>/&nbsp;<%=maxpage%> 页，每页 <%=pagesize%> 条 <a href="<%= Link.url("/app_mgr/ziliao_mgr/","log_downsearch.aspx","1","",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8) ,"","") %>">首页</a> <a href="<%= Link.url("/app_mgr/ziliao_mgr/","log_downsearch.aspx",(nowpage-1).ToString(),"",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8) ,"","") %>">上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/ziliao_mgr/","log_downsearch.aspx",(nowpage+1).ToString(),"",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8) ,"","") %>">下一页</a> <a href="<%= Link.url("/app_mgr/ziliao_mgr/","log_downsearch.aspx",maxpage.ToString(),"",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8) ,"","") %>">尾页</a></td>
     </tr>
   </table>
