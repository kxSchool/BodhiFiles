...
<script runat="server">
  ...
  public string contentstring;
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        sqls = " select tbdatarecycle.*,tbdatatype.id as typeid,tbdatatype.name as typename,tbuser.userno,tbuser.username from (( tbdatarecycle left join tbdatatype on  tbdatarecycle.type_id=tbdatatype.id ) left join tbuser on tbdatarecycle.userno=tbuser.userno) order by tbdatarecycle.uptime desc";
 ...
 && newext != "wps" && newext != "wpt" && newext != "xls" && newext != "xlt" && newext != "xml" && newext != "zip") { newext = "unknown"; }
    ...
    protected void ResumeDelete(DBClass db)
    {
        ...
        for (int i = 0; i < setfilestable.Rows.Count; i++)
        {
            ...
            frompath = ConfigurationSettings.AppSettings["delete_center"].ToString() + setfilestable.Rows[i]["fileurl"].ToString();
            topath = ConfigurationSettings.AppSettings["data_center"].ToString() + setfilestable.Rows[i]["fileurl"].ToString();
        ...
        db.closedb();
        Fun.alert("","恢复资料成功",Link.url("/app_mgr/ziliao_mgr/", "data_recycle.aspx", "", "", "", "", ""),"返回");
        Response.End();
    }
    ...
    protected void RealDelete(DBClass db)
    {
        ...
        //彻底删除的记录放到  彻底删除表中  tbdeldata
        string copy_sql;
        for (int ii = 0; ii < setfilestable.Rows.Count; ii++)
        {
            copy_sql = "insert into tbdeldata(filetitle,fileurl,description,filesizes,uptime,userno,popedom,type_id,deletetime) values(?,?,?,?,?,?,?,?,?)";
            OdbcParameter[] param ={ 
                      new OdbcParameter("@filetitle",OdbcType.VarChar),
                      new OdbcParameter("@fileurl",OdbcType.VarChar),
                      new OdbcParameter("@description",OdbcType.Text),
                      ...
                      new OdbcParameter("@popedom",OdbcType.Int),
                      new OdbcParameter("@type_id",OdbcType.Int),
                      new OdbcParameter("@deletetime",OdbcType.DateTime)
        ...
        string deletesql = "delete from tbdatarecycle where fileurl in('";
        foreach (string u in delids)
        {
            deletesql += u + "','";
        ...
        string filepath,realdeletepath;

        FileInfo ff, toff;
        foreach (string u in delids)
        {
            filepath = ConfigurationSettings.AppSettings["delete_center"].ToString() + u.ToString();
            realdeletepath = ConfigurationSettings.AppSettings["realdelete_center"].ToString() + u.ToString();

            ff = new FileInfo(filepath);
            toff = new FileInfo(realdeletepath);
            //先检测如果要移动的文件夹内已经存在该文件 则删除要移动的文件 不需要再移动
            if (toff.Exists)
            {
                if (ff.Exists)
            ...
            else
            {
                if (ff.Exists)
                {
                    File.Move(filepath, realdeletepath);
                }
            }
        ...
        db.closedb();
        Fun.alert("","删除成功",Link.url("/app_mgr/ziliao_mgr/", "data_recycle.aspx", "", "", "", "", ""),"返回");
        Response.End();
    }
...
<myhead:myhead ID="Myhead1" runat="server" />
  ...
  <tr>
    ...
    <td nowrap class=small><table>
      <tr>
        <td align="left" valign="middle" class="calendarNav" style="height: 16px">单位资料回收站
          </td>
        </tr>
    </table> </td>
  </tr>
...
<td >
 			...
 			<form action="<%= Link.url("/app_mgr/ziliao_mgr/","data_recycle.aspx","","set","","","")%>" method="post" name="selform" >
                     ...
                     <tr>
                         <td width="100%" align="center" valign="top">
                             <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                    ...
                                    <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_manage.aspx","","","","","") %>"><img src="/images/goback.jpg" style="border:0;" alt="返回资料管理" /></a>
									<%
										 if (Request.ServerVariables["LOCAL_ADDR"]==Request.ServerVariables["REMOTE_ADDR"])
										 {%>
										 <a  onclick="{if(confirm('此操作将资料移动到彻底删除资料中心，系统不可恢复！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='realdelete';document.selform.submit();return true;}return false;}" href="#"><img src="/images/realdelete.jpg" style="border:0;" alt="彻底删除" /></a>
										 <%}%>	
										 <a  onclick="{if(confirm('此操作将恢复资料！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='resumedelete';document.selform.submit();return true;}return false;}" href="#"><img src="/images/resumedata.jpg" style="border:0;" alt="恢复资料" /></a>
                                    </td>
                                 </tr>
                           </table>
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1" class="lvt small">
          ...
          <tr class="lvtCol"> 
			...
			<td align="center" style="height: 18px; width:12%;">
                资料类型</td>
            <td align="center" style="height: 18px; width:9%;">
                拥有者</td>
      ...
      共  <%=total%> 个资料，当前第 &nbsp;<font color="red"><%=nowpage%></font>/   <%=maxpage%>
            ...
            页，每页 
          <%=pagesize%>个&nbsp; <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_recycle.aspx","1","","","","") %>">首页</a> <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_recycle.aspx",(nowpage-1).ToString(),"","","","") %>">上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_recycle.aspx",(nowpage+1).ToString(),"","","","") %>">下一页</a>&nbsp;
          <a href="<%= Link.url("/app_mgr/ziliao_mgr/","data_recycle.aspx",maxpage.ToString(),"","","","") %>">尾页</a></td>
