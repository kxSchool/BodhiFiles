...
<script runat="server">
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        sqls = " select tbnews.*,tbnewstype.typename  from tbnews left join tbnewstype on  tbnews.type_id=tbnewstype.id where tbnews.isdelete=1 and tbnews.userno='" + Session["userno"].ToString() + "'  order by tbnews.createtime desc";
    ...
    protected void ResumeDelete(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","恢复信息成功！",Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx","","","","","") ,"返回");
        Response.End();
    }
...
<tr>
<td >
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx","","set","","","") %>" method="post" name="selform" >
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="0"
                     ...
                     <tr>
                         <td width="100%" align="center" valign="top">
                             <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                    ...
                                    <a onClick="javascript:history.go(-1);" href="#"><img src="/images/goback.jpg" style="border:0;" alt="返回资料管理" /></a><a  onclick="{if(confirm('此操作将恢复资料！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='resumedelete';document.selform.submit();return true;}return false;}" href="#"><img src="/images/resumedata.jpg" style="border:0;" alt="恢复资料" /></a><a  onclick="{if(confirm('此操作将彻底删除信息！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='realdelete';document.selform.submit();return true;}return false;}" href="#"></a>
                                    </td>
                                 </tr>
                           </table>
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
      ...
      共  <%=total%> 个资料，当前第 &nbsp;<font color="red"><%=nowpage%></font>/   <%=maxpage%>
            ...
            页，每页 
          <%=pagesize%>个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx","1","","","","") %>">首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx",(nowpage-1).ToString(),"","","","") %>">上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx",(nowpage+1).ToString(),"","","","") %>">下一页</a>&nbsp;
          <a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx",maxpage.ToString(),"","","","") %>>">尾页</a></td>
