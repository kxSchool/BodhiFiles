...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            sqls = "select * from tbnewstype  order by createtime desc";
        ...
        for (int i = (nowpage - 1) * pagesize; i < showcount; i++)
        {
            ...
            if (typetable.Rows[i]["ispublic"].ToString() == "0")
                sb.Append("<td height=\"24\" align=\"center\" >私有</td>\n");
            if (typetable.Rows[i]["ispublic"].ToString() == "1")
                sb.Append("<td height=\"24\" align=\"center\" ><font color=\"red\">公有</font></td>\n");
			if (typetable.Rows[i]["ispublic"].ToString() == "2")
                sb.Append("<td height=\"24\" align=\"center\" ><font color=\"bule\">全局</font></td>\n");
            sb.Append("<td height=\"24\" align=\"center\" ><a href=\""+Link.url("/app_mgr/news_mgr/","news_typesort.aspx",typetable.Rows[i]["id"].ToString(),"","","","") + "\">置顶 </a></td>\n");
            sb.Append("<td height=\"24\" align=\"center\" ><a href=\""+Link.url("/app_mgr/news_mgr/","news_typeedit.aspx",typetable.Rows[i]["id"].ToString(),"","","","") + "\">编辑</a></td>\n");
    ...
    protected void AddNewsType(DBClass db)
    {
        ...
        //检测不能有同名  
        string checksql = "select * from tbnewstype where typename= ?";
        ...
        param[2].Value = "0";
        string insertsql = "insert into tbnewstype(typename,createtime,ispublic)values(?,?,?)";
    ...
    protected void DeleteNewsType(DBClass db)
    {
        ...
        else
        {
            ...
            string deletesql = "delete from tbnewstype where id in(" + Request.Form.Get("sel").ToString() + ")";
...
<myhead:myhead ID="Myhead1" runat="server" />
   <div>
    <form name="form1" method="post" action="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx","","add","","","") %>" onsubmit ="return form_check();">
...
<td style="height: 103px" >
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx","","del","","","") %>" method="post" name="selform" >
      ...
      共 <%=total%> 条，当前第 &nbsp;<font color="red"><%=nowpage%></font>/&nbsp;<%=maxpage%>
            ...
            页，每页 
          <%=pagesize%> 条 <a href="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx","1","","","","") %>">首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx",(nowpage-1).ToString(),"","","","") %>">上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx",(nowpage+1).ToString(),"","","","") %>">下一页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_typemanage.aspx",maxpage.ToString(),"","","","") %>">尾页</a> &nbsp;&nbsp;&nbsp;&nbsp;
