...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        DataTable datatable = new DataTable();
            ...
            news_id = "";
sqls ="SELECT tbnews.id AS news_id, tbnews.news_title, tbnewsbbs.id, tbuser.UserName, tbnewsbbs.content, tbnewsbbs.createtime, tbUser_1.UserName as newsusername";
sqls =sqls + " FROM ((tbnewsbbs LEFT JOIN tbnews ON tbnewsbbs.news_id = tbnews.id) LEFT JOIN tbuser ON tbnewsbbs.userno = tbuser.UserNo) left JOIN tbUser AS tbUser_1 ON tbnews.userno = tbUser_1.UserNo";
sqls =sqls + " where tbnewsbbs.isdelete=0 and tbnews.isdelete=0 ORDER BY tbnewsbbs.createtime DESC";
        ...
        for (int i = (nowpage - 1) * pagesize; i < showcount; i++)
        {
			...
			sb.Append("<font class=\"fontBold\">　发信人:</font>" + datatable.Rows[i]["newsusername"].ToString() + "");
    ...
    protected void DeleteNewsbbs(DBClass db)
    {
        ...
        db.closedb();
			   ...
			   Fun.alert("","删除成功！",Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",Request.QueryString["param1"].ToString(),"","","","") ,"返回");
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx","","","","","") ,"返回");
        Response.End();
    }
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,"","set","","") %>" method="post" name="selform2" >
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,"1","set","","") %>" method="post" name="selform" >

     <table width="100%" border="0" cellpadding="5" cellspacing="1"  class="lvt small">
         <tr class="lvtCol">
             <td height="30" align="center">
                 序号</td>
             <td align="center" style="height: 18px;">
                 <input type="checkbox" name="checkbox" value="checkbox" onClick="javascript:SelectAll()"></td>
             <td align="center" style="height: 18px;">信息名称 回复内容 回复人/发布时间</td>
             </tr>
        ...
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="8" height="35" class="but1" >                 <input type="hidden" id="actiontype" name="actiontype" style="width: 47px; height: 20px" />
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,"1","","","") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,(nowpage-1).ToString(),"","","") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,(nowpage+1).ToString(),"","","") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_alllog.aspx",news_id,maxpage.ToString(),"","","") %>">
                     尾页</a></td>
         </tr>
     </table>   
