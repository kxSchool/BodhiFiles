...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        DataTable datatable = new DataTable();
            ...
            sqls = "select tbnewsLog.id,tbnewsLog.loginnewsid,tbuser.username,tbnewsLog.ip,tbnewsLog.browser,tbnewsLog.os,tbnewsLog.logintime from (tbnewsLog left join tbnews on tbnews.id=tbnewsLog.loginnewsid) left join tbuser on tbnewsLog.loginuserno=tbuser.userno where  tbnews.id=? order by tbnewsLog.logintime desc";
    ...
    protected void DeleteNewsbbs(DBClass db)
    {
        ...
        db.closedb();
			   ...
			   Fun.alert("","删除成功！",Link.url("/app_mgr/news_mgr/","news_log.aspx",Request.QueryString["param1"].ToString(),"","","","") ,"返回");
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","news_log.aspx","","","","","") ,"返回");
        Response.End();
    }
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,"","set","","") %>" method="post" name="selform2" >
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,"1","set","","") %>" method="post" name="selform" >
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
        ...
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="12" height="35" class="but1" style="width: 100%;">                 <input type="hidden" id="actiontype" name="actiontype" style="width: 47px; height: 20px" />
                 共
                 <%=total%>
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,"1","","","") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,(nowpage-1).ToString(),"","","") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,(nowpage+1).ToString(),"","","") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/news_mgr/","news_log.aspx",news_id,maxpage.ToString(),"","","") %>">
                     尾页</a></td>
         </tr>
     </table>   
