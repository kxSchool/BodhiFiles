...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        DataTable datatable = new DataTable();
            ...
            news_id ="";
            sqls = "select tbnews.id as news_id,tbnewsLog.id,tbnews.news_title,tbnewsLog.loginnewsid,tbuser.username,tbnewsLog.ip,tbnewsLog.browser,tbnewsLog.os,tbnewsLog.logintime from (tbnewsLog left join tbnews on tbnews.id=tbnewsLog.loginnewsid) left join tbuser on tbnewsLog.loginuserno=tbuser.userno  order by tbnewsLog.logintime desc";
        ...
        for (int i = (nowpage - 1) * pagesize; i < showcount; i++)
        {
            ...
            sb.Append("<td height=\"24\" align=\"left\" ><a  href=\""+Link.url("/app_mgr/news_mgr/","news_view.aspx",datatable.Rows[i]["news_id"].ToString(),"","","","") + "\"> &nbsp;&nbsp;" + datatable.Rows[i]["news_title"].ToString() + "</a></td>\n");
			sb.Append("<td height=\"24\" align=\"left\" >" + datatable.Rows[i]["username"].ToString() + "</td>\n");
            sb.Append("<td height=\"24\" align=\"center\" >" + datatable.Rows[i]["logintime"].ToString() + "</a></td>\n");
            sb.Append("<td height=\"24\" align=\"center\" >" + datatable.Rows[i]["browser"].ToString() + "</td>\n");
            sb.Append("<td height=\"24\" align=\"center\" >" + datatable.Rows[i]["os"].ToString() + "</td>\n");
            sb.Append("<td height=\"24\" align=\"center\" ><a href=\"http://iplocal.js715.net/tool/iplocal/ip138.html?id=" + datatable.Rows[i]["ip"].ToString() + "\" target=\"_blank\"><script language=\"javascript\" type=\"text/javascript\" src=\"http://iplocal.js715.net/tool/iplocal/default.html?id=" + datatable.Rows[i]["ip"].ToString() + "\"></" + "script" + "></a></td>\n");
            sb.Append("</tr>\n");
    ...
    protected void DeleteNewsbbs(DBClass db)
    {
        ...
        //删除原表
        string delete_sql = "delete from  tbnewsLog  where tbnewsLog.id in (" + Request.Form.Get("sel").ToString() + ")";
        db.ExecuteNonQuery(delete_sql, null);
        db.closedb();


			   Fun.alert("","删除成功！",Link.url("/app_mgr/news_mgr/","news_alllog.aspx",Request.QueryString["param1"].ToString(),"","","","") ,"返回");
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","news_alllog.aspx","","","","","") ,"返回");
        Response.End();
    }
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,"","set","","") %>" method="post" name="selform2" >
  <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
  <tr>
    <td height="32" class=small style="width:50px">&nbsp;</td>
    <td width="100" nowrap class=small><table>
        <tr>
          <td> <b><font color="#ffffff">信息阅览日志</font></b> <a class="redword" onClick="initialize(1,'1',event);ShowHide('1',null);return false;"></a></td>
        </tr>
    </table></td>
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,"1","set","","") %>" method="post" name="selform" >
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
         <tr class="lvtCol">
             ...
             <td align="center" style="height: 18px;">信息标题</td>
             <td align="center" style="height: 18px;">
                 阅览者</td>
                  <td align="center">
                 阅览时间</td>
             <td align="center">浏览器</td>
				  ...
				  <td align="center">访问来源地理位置</td>
              
         </tr>
          
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="13" height="35" class="but1" style="width: 100%;">                 <input type="hidden" id="actiontype" name="actiontype" style="width: 47px; height: 20px" />
                 共
                 <%=total%>
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,"1","","","") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,(nowpage-1).ToString(),"","","") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,(nowpage+1).ToString(),"","","") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/news_mgr/","news_alllog.aspx",news_id,maxpage.ToString(),"","","") %>">
                     尾页</a></td>
         </tr>
     </table>   
