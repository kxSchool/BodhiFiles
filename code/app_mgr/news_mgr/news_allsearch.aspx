...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["param2"] == null || Request.QueryString["param2"] == "")
        {
		   ...
		   Fun.alert("","你没有输入查询条件！",Link.url("/app_mgr/news_mgr/","news_manage.aspx","","","","","") ,"返回");
            Response.End();
    ...
    void ShowPage(DBClass db)
    {
        ...
        if (newstype == "")
        {
             if (dept_id == "")
        {
		 sqls = " select tbnews.*,tbuser.username,tbnewstype.typename from (tbnews left join tbnewstype on  tbnews.type_id=tbnewstype.id) left join tbuser on tbnews.userno=tbuser.userno where (tbnews.isdelete=0 and   (tbnews.news_title like '%" + condition + "%' or  tbuser.username like '%" + condition + "%' or  tbnews.content like '%" + condition + "%'))";
        }
		else
		{
		 sqls = " select tbnews.*,tbuser.username,tbnewstype.typename from (tbnews left join tbnewstype on  tbnews.type_id=tbnewstype.id) left join tbuser on tbnews.userno=tbuser.userno where (tbnews.isdelete=0 and  tbuser.dept_id= " + dept_id + " and   (tbnews.news_title like '%" + condition + "%' or  tbuser.username like '%" + condition + "%' or  tbnews.content like '%" + condition + "%'))";
		}
        ...
        {
            sqls = " select tbnews.*,tbuser.username,tbnewstype.typename from (tbnews left join tbnewstype on  tbnews.type_id=tbnewstype.id)  left join tbuser on tbnews.userno=tbuser.userno where (tbnews.isdelete=0 and  (tbnews.news_title like '%" + condition + "%' or  tbuser.username like '%" + condition + "%' or  tbnews.content like '%" + condition + "%')) and tbnews.type_id=" + newstype;
        		Session["condition"] ="null";
		}
...
if(Session["condition"]==null) {Session["condition"]="null";} 		
			...
			sb.Append("&nbsp;<a  href=\""+Link.url("/app_mgr/news_mgr/","news_view.aspx",datatable.Rows[i]["id"].ToString(),"","","","") + "\">" + datatable.Rows[i]["news_title"].ToString().Replace(Session["condition"].ToString(),"<font color=blue>" + Session["condition"].ToString() + "</font>")  + "</a></td>\n");
...
<td >
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","","set","","","") %>" method="post" name="selform" >
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0"
                         ...
                         <td width="100%" align="center" valign="top">
                             <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                 ...
                                 <td>
                                    ...
                                    <a onClick="javascript:history.go(-1);" href="#"><img src="/images/goback.jpg" style="border:0;" alt="" /></a><a href="<%=Link.url("/app_mgr/news_mgr/","news_add.aspx","","","","","") %>"><img src="/images/addnews.jpg" style="border:0;" alt="" /></a>
									<%if (1==2) {%>
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
      ...
      共  <%=total%> 个资料，当前第 &nbsp;<font color="red"><%=nowpage%></font>/   <%=maxpage%>
            ...
            页，每页 
          <%=pagesize%>个&nbsp; <a href="<%= Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","1","search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),newstype,dept_id) %>">首页</a> <a href="<%= Link.url("/app_mgr/news_mgr/","news_allsearch.aspx",(nowpage-1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),newstype,dept_id) %>">上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/news_mgr/","news_allsearch.aspx",(nowpage+1).ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),newstype,dept_id) %>">下一页</a>&nbsp;
          <a href="<%= Link.url("/app_mgr/news_mgr/","news_allsearch.aspx",maxpage.ToString(),"search",HttpUtility.UrlEncode(condition, System.Text.Encoding.UTF8),newstype,dept_id) %>">尾页</a></td>
