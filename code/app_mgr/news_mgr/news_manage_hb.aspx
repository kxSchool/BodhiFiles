...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        {
            type_idd = "";
            sqls = "select tbnews.*,tbuser.username,tbuser.dept_id,department.deptname from ((tbnews  left join tbuser  on tbnews.userno=tbuser.userno) left join department on tbuser.dept_id=department.id )  where tbnews.isdelete=0  and  tbnews.type_id=12 ";
        ...
        //填充下拉框
        DataTable typetable = db.GetTable("select * from tbnewstype where id=12", null);
    ...
    protected void DeleteNews(DBClass db)
    {
        ...
        Response.Redirect(Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","","","","",""));
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","","","","","") ,"返回");
        Response.End();
    }
</script>
...
{
	 url="<%=Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","","search","","type_idd","") %>";
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","news_allsearch.aspx","","search","","","") %>" method="post" name="selform2" >
  <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
  <tr>
    <td height="32" class=small style="width:50px">&nbsp;</td>
    <td width="100" nowrap class=small><table>
        <tr>
          <td> <b><font color="#ffffff">工作汇报</font></b></td>
        </tr>
    </table></td>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
                     ...
                     <tr>
                         <td width="100%" align="center" valign="top">
                             <%if (1==2) {%><table width="100%" border="0" cellpadding="0" cellspacing="0">
									 ...
									 <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                       <tr>
                                         <td align="left"><a onClick="{if(confirm('此操作将删除信息至回收站！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='delete';document.selform.submit();return true;}return false;}" href="#"><img src="/images/deletefile.jpg" style="border:0;" alt="" /></a><a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx","","","","","") %>"></a> </td>
                                       </tr>
                                     </table>
                                   </td>
                                 </tr>
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_manage_hb.aspx","","set","","","") %>" method="post" name="selform" >

     <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="lvt small">
         <tr class="lvtCol">
             <td width="49" height="30" align="center">
                 序号</td>
             <td width="37" align="center" style="height: 18px;">
        ...
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="10" height="35" class="but1" style="width: 100%;">
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%= Link.url("/app_mgr/news_mgr/","news_manage_hb.aspx","1","","",type_idd,"") %>">
                     首页</a> <a href="<%= Link.url("/app_mgr/news_mgr/","news_manage_hb.aspx",(nowpage-1).ToString(),"","",type_idd,"") %>">
                         上一页</a> &nbsp;<a href="<%= Link.url("/app_mgr/news_mgr/","news_manage_hb.aspx",(nowpage+1).ToString(),"","",type_idd,"") %>">下一页</a>&nbsp;
                 <a href="<%= Link.url("/app_mgr/news_mgr/","news_manage_hb.aspx",maxpage.ToString(),"","",type_idd,"") %>">
                     尾页</a></td>
         </tr>
     </table>   
