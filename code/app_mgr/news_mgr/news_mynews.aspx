...
<script runat="server">
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-7).ToShortDateString().ToString());
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (Request.QueryString["param2"].ToString() == "search")
            {
                ...
                Response.Redirect(Link.url("/app_mgr/news_mgr/","news_mynewssearch.aspx","","search",condition, newstype,""));
            }
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        if (Request.QueryString["param4"] == null || Request.QueryString["param4"]== "")
        {
            type_idd = "";
            sqls = "select tbnews.*,tbnewstype.id as typeid,tbnewstype.typename from tbnews left join tbnewstype on tbnews.type_id=tbnewstype.id  where tbnews.isdelete=0 and  tbnews.userno=?  ";

        }
        else
        {
            type_idd = Request.QueryString["param4"].ToString();
            sqls = "select tbnews.*,tbnewstype.id as typeid,tbnewstype.typename from tbnews left join tbnewstype on tbnews.type_id=tbnewstype.id  where tbnews.isdelete=0 and tbnews.userno=? and tbnews.type_id=" + type_idd;
        }
    ...
    protected void DeleteNews(DBClass db)
    {
        ...
        db.closedb();
			   ...
			   Fun.alert("","删除成功！",Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","","","","") ,"返回");
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","","","","") ,"返回");
        Response.End();
    }
...
{
	 url="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","","","type_idd","") %>";
...
function allsearch()
{
	 url="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","search","param3_idd","type_idd","") %>";
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","search","","","") %>" method="post" name="selform2" >
  <TABLE border=0 cellspacing=0 cellpadding=0 width=100% class="hdrTabBg">
  <tr>
    <td height="32" class=small style="width:50px">&nbsp;</td>
    <td width="100" nowrap class=small><table>
        <tr>
          <td> <b><font color="#ffffff">我发布的信息</font></b></td>
        </tr>
    </table></td>
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
                         ...
                         <td width="100%" align="center" valign="top">
                             <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                     ...
                                     <td align="left" valign="middle" bgcolor="#FFFFFF"  style="width: 845px; height: 26px"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                                       <tr>
                                         <td align="left"><a href="<%=Link.url("/app_mgr/news_mgr/","news_add.aspx","","","","","") %>"><img src="/images/addnews.jpg" style="border:0;" alt="" /></a><a onClick="{if(confirm('此操作将删除信息至回收站！\n\n确定要执行此项操作吗？')){document.getElementById('actiontype').value='delete';document.selform.submit();return true;}return false;}" href="#"><img src="/images/deletefile.jpg" style="border:0;" alt="" /></a><a href="<%=Link.url("/app_mgr/news_mgr/","news_myrecycle.aspx","","","","","") %>"><img src="/images/recy.jpg" style="border:0px;" /></a> </td>
                                       </tr>
                                     </table>
                                   </td>
                                 </tr>
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx","","set","","","") %>" method="post" name="selform" >
        ...
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="10" height="35" class="but1" style="width: 100%;">
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx","1","","",type_idd,"") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx",(nowpage-1).ToString(),"","",type_idd,"") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx",(nowpage+1).ToString(),"","",type_idd,"") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/news_mgr/","news_mynews.aspx",maxpage.ToString(),"","",type_idd,"") %>">
                     尾页</a></td>
         </tr>
     </table>   
