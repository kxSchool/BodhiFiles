...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void ShowPage(DBClass db)
    {
        ...
        DataTable datatable = new DataTable();
            ...
            sqls = "select tbnewsbbs.id,tbuser.username,tbnewsbbs.content,tbnewsbbs.createtime from (tbnewsbbs left join tbnews on tbnews.id=tbnewsbbs.news_id ) left join tbuser on tbnewsbbs.userno=tbuser.userno where tbnewsbbs.isdelete=0 and tbnews.id=? order by tbnewsbbs.createtime desc";
        ...
        for (int i = (nowpage - 1) * pagesize; i < showcount; i++)
        {
            ...
            sb.Append("<td height=\"50\" align=\"center\" >" + id.ToString() + "</td>\n");
            ...
            sb.Append("<td height=\"150\" align=\"left\"  valign=\"top\">" + datatable.Rows[i]["content"].ToString() + "\n");
            sb.Append("<font class=\"fontBold\">" + datatable.Rows[i]["username"].ToString() + "\n");
            sb.Append("" + datatable.Rows[i]["createtime"].ToString() + "</font></td>\n");
    ...
    protected void DeleteNewsbbs(DBClass db)
    {
        ...
        db.closedb();
			   ...
			   Fun.alert("","删除成功！",Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",Request.QueryString["param1"].ToString(),"","","","") ,"返回");
        Response.End();
    ...
    protected void MoveType(DBClass db)
    {
        ...
        db.closedb();
		   ...
		   Fun.alert("","移动成功！",Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx","","","","","") ,"返回");
        Response.End();
    }
...
<myhead:myhead ID="Myhead1" runat="server" />
	<form action="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,"","set","","") %>" method="post" name="selform2" >
...
<td >
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0"
                         ...
                         <td width="100%" align="center" valign="top">
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                     ...
                                     <td align="left" valign="middle" bgcolor="#FFFFFF"  style="width: 845px; height: 26px"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                                       <tr>
                                         <td align="left">
										 <a onClick="javascript:history.go(-1);" href="#"><img src="/images/goback.jpg" style="border:0;" alt="" /></a><a class="redword" onClick="initialize(1,'1',event);ShowHide('1',null);return false;"><img src="/images/addnewsbbs.jpg" alt="" width="95" height="24" style="border:0;" /></a>
										<%
										 if ( Request.ServerVariables["LOCAL_ADDR"]==Request.ServerVariables["REMOTE_ADDR"])
										 {%>
 			...
 			<form action="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,"1","set","","") %>" method="post" name="selform" >
     ...
     <table width="100%" border="0" cellpadding="3" cellspacing="1"  class="lvt small">
        ...
        <%=contentstring %>
         <tr bgcolor="white" onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'">
             <td align="center" colspan="8" height="35" class="but1" style="width: 100%;">                 <input type="hidden" id="actiontype" name="actiontype" style="width: 47px; height: 20px" />
                 共
                 <%=total%>
                 ...
                 页，每页
                 <%=pagesize%>
                 个&nbsp; <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,"1","","","") %>">
                     首页</a> <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,(nowpage-1).ToString(),"","","") %>">
                         上一页</a> &nbsp;<a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,(nowpage+1).ToString(),"","","") %>">下一页</a>&nbsp;
                 <a href="<%=Link.url("/app_mgr/news_mgr/","newsbbs_log.aspx",news_id,maxpage.ToString(),"","","") %>">
                     尾页</a></td>
         </tr>
     </table>   
