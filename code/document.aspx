...
DateTime dt1=Convert.ToDateTime(System.DateTime.Now.AddDays(-1).ToShortDateString().ToString());
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        typetable = db.GetTable("select * from tbnewstype where ispublic='1' or ispublic='2' order by createtime desc");
        dt = db.GetTable("SELECT tbnews.*, tbnewstype.typename,tbuser.username FROM (tbnews LEFT JOIN tbnewstype ON tbnews.type_id = tbnewstype.id) LEFT JOIN tbUser ON tbnews.userno = tbUser.UserNo WHERE (((tbnews.isdelete)=0) AND ((tbnewstype.ispublic='1') or (tbnewstype.ispublic='2')) ) ORDER BY tbnewstype.typename, tbnews.createtime DESC",null);
       //string abc;
            ...
            <td valign="top" style="height: 140px;width:33%;font-size:12px;">
                <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#C0C0C0">
                    <tr>
					  ...
					  Response.Write("class=\"save\"");
					  }
					   else
					  { 
					  Response.Write("class=\"event\"");
					  }
					  %>
					   >
                        <tr>
                          <td class="txtGreen">　 <strong><%= typetable.Rows[i]["typename"].ToString() %></strong></td>
                    ...
                    <tr>
					  ...
					  for(ii = 0; ii < dt.Rows.Count; ii++)
						 {
						 if(typetable.Rows[i]["id"].ToString()==dt.Rows[ii]["type_id"].ToString() && sub_i<=5 )
						        {%>
								<tr><td height="20">
						<IMG height="12" 
